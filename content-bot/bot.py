...
async def _periodic_sheets_poll():
    """Один опрос листа «Посты» — ловит ручные правки статуса на «На согласование», «🚨 Срочно»
    и «К публикации» (последнее — иначе расписание подхватывается только раз в сутки в 03:00)."""
    loop = asyncio.get_running_loop()
    try:
    ...
    for item in pending["to_publish"]:
        gen = db.get_generation(item["gen_id"])
        if not gen or gen["status"] in ("to_publish", "published"):
            continue  # уже запланирован или опубликован

        clean = item["pub_date"].split("(")[0].strip()
        dt = None
        for fmt in ("%d.%m.%Y %H:%M", "%d.%m.%Y"):
            try:
                ...
                continue
        if dt is None:
            logging.warning(f"[sheets] не удалось разобрать дату '{item['pub_date']}' для gen_id={item['gen_id']}")
            continue
        ...
        try:
            ...
            if dt <= now:
                ...
                publisher.schedule_generation(item["gen_id"], dt)
        except Exception as e:
            logging.warning(f"[bot] постановка в расписание не удалась (gen_id={item['gen_id']}): {e}")
