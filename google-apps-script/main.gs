...
function readMonblan_(weekNum, year) {
  try {
    ...
    // Попытка 2: «ЕжеНедельно» хранит порядковые недели, не ISO.
    // Год в строке 1 может опережать календарный (неделя 53 → 2027).
    // Берём последнюю заполненную колонку независимо от года.
    if (mbCol === -1) {
      Logger.log('  ⚠️ Монблан: неделя ' + weekNum + '/' + year + ' не найдена, берём последнюю заполненную');
      for (var j = weekRow.length - 1; j >= 0; j--) {
        if (weekRow[j] !== '' && weekRow[j] !== null) {
          mbCol = j + 2;
          Logger.log('  ℹ️ Монблан: последняя кол.' + mbCol + ' (нед.' + weekRow[j] + ', год ' + yearRow[j] + ')');
          break;
        }
      }
    }
