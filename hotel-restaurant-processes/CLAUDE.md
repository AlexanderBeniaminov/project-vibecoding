...
Динамическое сравнение выбранной недели 2026 vs 2025. Управляется через `monblan-gas/monblan_dashboard.gs`.

**Принцип работы:**
- Читает ВСЕ 96 строк листа «Монблан» напрямую
- Вычисляет дельту (2026−2025)/|2025| для каждой метрики
- Показывает ТОЛЬКО строки с отклонением — не ⚪
- Количество строк динамическое: 3 отклонения → 3 строки; 25 → 25 строк

**Пороги отклонений:**
- 🔴 падение > 10% (T_RED = −0.10)
- 🟡 падение 3–10% (T_YELLOW = −0.03)
- 🟢 рост > 5% (T_GREEN = +0.05)
- ⚪ норма (−3% … +5%) — **не показывается**

**Структура дашборда (строки):**
- Строка 1 — заголовок «ДАШБОРД — МОНБЛАН»
- Строка 2 — B2: выбор недели (выпадающий список 1–5)
- Строка 3 — шапка таблицы (Показатель | 2025 | 2026 | Δ% | ●)
- Строки 4..N — динамические строки отклонений (только те, что не ⚪)
- Строки N+2..N+14 — AI-блок: Причины (3 строки) + Рекомендации (3) + Выводы (3)

**Как работает GAS:**
1. Пользователь выбирает неделю в B2 → срабатывает `onEditDashboard` → вызывает `refreshDashboard()`
2. `findWeekCol_()` ищет столбец в листе Монблан где строка1=год И строка2=неделя
3. `getDeviations_()` читает 96 строк, считает дельты, фильтрует не-⚪
4. `writeDeviationRows_()` пишет динамическое количество строк с цветовой индикацией
...
- Читает отклонения напрямую из листа Монблан (не из дашборда)
- AI-блок позиционируется динамически: сразу после последней строки отклонений + 2
