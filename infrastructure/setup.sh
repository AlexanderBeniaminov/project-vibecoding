...
echo "║   VLESS+Reality (3X-UI) + Python + Parsers + Bots      ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo ""
...
# ufw allow ${WG_PORT}/udp comment 'WireGuard (ЗАМЕНЕНО на VLESS)'
ufw --force enable
log "Firewall активен: порты 22 (SSH) и 443/TCP (VLESS-Reality)"

# ════════════════════════════════════════════════════════════
# 4. VLESS + XTLS-REALITY (3X-UI) — ТЕКУЩИЙ VPN
# ════════════════════════════════════════════════════════════
log "Установка 3X-UI (Xray панель)..."
bash <(curl -Ls https://raw.githubusercontent.com/mhsanaei/3x-ui/master/install.sh)
log "3X-UI установлен. Панель: http://${SERVER_IP}:54321"
log "Следующий шаг: открой браузер → настрой VLESS+Reality inbound → экспортируй vless:// ссылки"
warn "Не забудь: ufw delete allow 54321/tcp — после настройки панели!"

# ════════════════════════════════════════════════════════════
# 4b. WIREGUARD VPN (УСТАРЕЛО — ЗАМЕНЕНО НА VLESS+Reality)
# ════════════════════════════════════════════════════════════
# Этот блок сохранён для справки. На новых серверах не выполнять.
if false; then
...
ANDROID_PRIV=$(cat android_private.key)
ANDROID_PUB=$(cat android_public.key)
ANDROID_PSK=$(cat android_psk.key)
...
# ── Клиент: android (Huawei Nova 11) ─────────────────────
[Peer]
PublicKey    = ${ANDROID_PUB}
PresharedKey = ${ANDROID_PSK}
AllowedIPs   = ${WG_ANDROID_IP}/32
EOF
...
cat > /etc/wireguard/clients/android.conf << EOF
[Interface]
PrivateKey = ${ANDROID_PRIV}
Address    = ${WG_ANDROID_IP}/32
DNS        = 1.1.1.1, 8.8.8.8
...
qrencode -t png -o /etc/wireguard/clients/android_qr.png < /etc/wireguard/clients/android.conf

# ════════════════════════════════════════════════════════════
# 5. WATCHDOG для WireGuard (УСТАРЕЛО — см. секцию 4)
# ════════════════════════════════════════════════════════════
...
# ═══ конец блока WireGuard (УСТАРЕЛО) ═══
...
# Ежедневный отчёт в VK MAX — 06:00 UTC = 09:00 МСК
0 6 * * * /home/parser/venv/bin/python /home/parser/bots/daily_report.py alex >> /home/parser/logs/daily_report_\$(date +\%Y-\%m-\%d).log 2>&1
...
echo "╚══════════════════════════════════════════════════════════╝"
echo ""
echo "🔒 VPN: VLESS + XTLS-Reality (3X-UI)"
echo "   Панель: http://${SERVER_IP}:54321"
echo "   Клиент на Mac/Android: Hiddify (hiddify.com)"
echo ""
echo "🔧 Следующие шаги — VPN:"
echo "   1. Открой http://${SERVER_IP}:54321 → настрой VLESS+Reality inbound"
echo "   2. Добавь клиентов: alex, oleg, android → скопируй vless:// ссылки"
echo "   3. Закрой порт панели: ufw delete allow 54321/tcp && ufw reload"
echo "   4. Установи Hiddify на Mac → Add Profile → вставь vless:// ссылку"
echo ""
...
echo ""
echo "🔧 Следующие шаги — парсеры:"
echo "   1. Заполни settings.py для каждого аккаунта"
echo "   2. Загрузи service_account.json в config/alex/ и config/oleg/"
echo ""
echo "🔍 Статус 3X-UI:"
