...
function buildMonblanSheet(opt_silent, opt_ss) {
  ...
  // ⚠️ ЗАЩИТА: buildMonblanSheet ПОЛНОСТЬЮ УДАЛЯЕТ ВСЕ ДАННЫЕ листа «ЕжеНедельно».
  // Запускать только при первичной настройке или после восстановления из резервной копии.
  // Для восстановления данных используй: Файл → История версий → Восстановить
  if (!opt_silent) {
    var ui = SpreadsheetApp.getUi();
    var result = ui.prompt(
      '⚠️ ОПАСНАЯ ОПЕРАЦИЯ',
      'buildMonblanSheet УДАЛИТ ВСЕ ИСТОРИЧЕСКИЕ ДАННЫЕ листа «ЕжеНедельно»!\n\n' +
      'Для восстановления данных лучше использовать:\n' +
      'Файл → История версий → Восстановить предыдущую версию\n\n' +
      'Если всё равно хочешь продолжить — напечатай слово  УДАЛИТЬ  и нажми OK.',
      ui.ButtonSet.OK_CANCEL
    );
    if (result.getSelectedButton() !== ui.Button.OK || result.getResponseText().trim() !== 'УДАЛИТЬ') {
      ui.alert('Отменено. Данные не изменены.');
      return;
    }
