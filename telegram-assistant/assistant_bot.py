...
async def run_llm(history: list[dict], user_id: int, chat_id: int) -> str:
    ...
    system_msg = {
        ...
        "content": (
            ...
            "② Упомянута дата или время + действие → add_reminder.\n"
            "   Алиасы времени: «утро»=09:00, «день»=14:00, «вечер»=18:00,\n"
            "   «срочно»/«сейчас»=текущее время+2 минуты, «ночь»=22:00.\n"
            "   Если указана только дата без времени — используй 09:00 по умолчанию.\n"
            ...
            "   Если время не указано — используй 09:00. Не спрашивай «бессрочно?» — повторяется до ручной отмены.\n\n"
            ...
            "- НЕЛЬЗЯ спрашивать «во сколько напомнить?» — если нет времени, используй 09:00.\n"
...
async def handle_message(message: Message):
    ...
    try:
        response = await asyncio.wait_for(
            run_llm(history, user_id, message.chat.id),
            timeout=120,
        )
