...
async def run_llm(history: list[dict], user_id: int, chat_id: int) -> str:
    ...
    system_msg = {
        ...
        "content": (
            ...
            "ПРИОРИТЕТ 1 — если упомянут член команды (Виктор, Евгения, Надежда, Управляющий, Тех.директор) "
            "как исполнитель или адресат задачи/поручения → ВСЕГДА вызывай add_team_task (executor=имя, task=суть). "
            "Примеры: 'напомни Виктору про отчёты', 'скажи Евгении сделать X', 'поставь задачу Надежде' → add_team_task. "
            "ИСКЛЮЧЕНИЕ: если Александр хочет напомнить СЕБЕ о чём-то связанном с командой → add_reminder. "
            "Пример: 'напомни мне позвонить Виктору' → add_reminder (исполнитель Александр, не Виктор). "
            "ПРИОРИТЕТ 2 — если есть время/дата и нет члена команды → add_reminder; "
            "если задача/дело без члена команды → add_note с tags='задача'; "
