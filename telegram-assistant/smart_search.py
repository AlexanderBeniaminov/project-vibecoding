...
COMMERCIAL_RE = re.compile(
    r"\b(?:авиабилет|билет|перел[её]т|авиа\s|рейс|самол[её]т|"
    r"отел|гостиниц|забронируй|бронировать|суточн|ноч[её]вк|"
...
_SHOPPING_SYSTEM = """Ты агент сравнения цен. Ищешь цены на конкретных сайтах: wildberries.ru, ozon.ru, auchan.ru, metro-cc.ru, lenta.com, 5ka.ru, perekrestok.ru.

ЕСЛИ В ЗАПРОСЕ ОДИН ТОВАР — выведи список по магазинам (без дублирования):

🛒 [Название товара]

Wildberries — 485 ₽ (доставка бесплатно)
https://www.wildberries.ru/catalog/.../detail.aspx

Ozon — 520 ₽ (доставка 99 ₽, итого 619 ₽)
https://www.ozon.ru/product/.../

Лента — не найдено

...и так по каждому из 7 сайтов...

✅ Дешевле всего: Wildberries — 485 ₽ с доставкой

---
ЕСЛИ В ЗАПРОСЕ НЕСКОЛЬКО ТОВАРОВ — две секции:

🛒 Оптимальная корзина (каждый товар в самом дешёвом магазине)
Молоко — Пятёрочка — 89 ₽ (доставка бесплатно) — https://5ka.ru/...
Хлеб — Ozon — 65 ₽ (доставка 99 ₽) — https://ozon.ru/...
Итого: товары 154 ₽ + доставка 99 ₽ (Ozon) = 253 ₽

🏪 Всё в одном магазине — Ozon (дешевле всего)
Молоко — 95 ₽, Хлеб — 70 ₽
Итого: 165 ₽ + доставка 99 ₽ = 264 ₽

---
ЖЁСТКИЕ ПРАВИЛА:
- Ищи товар на КАЖДОМ из 7 сайтов — не пропускай магазины
- Ссылка — ПОЛНЫЙ URL начиная с https:// на конкретную страницу товара
- Доставка по умолчанию если неизвестна: Wildberries бесплатно, Ozon 99₽ (бесплатно от 2499₽), Ашан 199₽, Metro 299₽, Лента бесплатно от 1500₽, Пятёрочка бесплатно от 500₽, Перекрёсток бесплатно от 700₽
- Только реальные цены с учётом акций
- Сторонние сайты (доставка-продуктов.ру, яндекс.маркет и др.) — НЕ ПОКАЗЫВАЙ
- Пиши по-русски, суммы в рублях ₽"""
...
_IATA = {
    ...
    "геленджик": "GDZ", "геленджика": "GDZ", "геленджике": "GDZ",
    "геленджику": "GDZ",
    "симферополь": "SIP", "симферополя": "SIP", "симферополе": "SIP",
    "белгород": "EGO", "белгорода": "EGO", "белгороде": "EGO",
    "воронеж": "VOZ", "воронежа": "VOZ", "воронеже": "VOZ",
    "пенза": "PEZ", "пензы": "PEZ", "пензе": "PEZ",
    "саратов": "RTW", "саратова": "RTW", "саратове": "RTW",
    "махачкала": "MCX", "махачкалы": "MCX", "махачкале": "MCX",
    "грозный": "GRV", "грозного": "GRV", "грозном": "GRV",
    "нальчик": "NAL", "нальчика": "NAL", "нальчике": "NAL",
...
def detect_type(text: str) -> str:
    ...
    q = text.lower()
    if any(w in q for w in ["билет", "рейс", "авиа", "самолёт", "самолет",
                             "перелёт", "перелет", "авиабилет"]):
        return "flights"
    if any(w in q for w in ["отел", "гостиниц", "забронир", "суточ",
                             "ночёвк", "ночевк", "переночев", "проживан",
                             "где остановить"]):
        return "hotels"
    return "products"
...
async def smart_search_and_answer(
    ...
    if kind == "flights":
        ...
        # Явно перечисляем магазины чтобы Perplexity искал в каждом
        user_query = (
            query
            + " wildberries.ru ozon.ru auchan.ru metro-cc.ru lenta.com 5ka.ru perekrestok.ru"
        )
    ...
    try:
        resp = await ai_client.chat.completions.create(
            ...
            max_tokens=2000 if kind == "products" else 600,
            temperature=0.1,
        )
