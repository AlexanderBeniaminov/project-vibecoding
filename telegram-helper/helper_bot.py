    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": (
                ...
                "ПРАВИЛА: "
                "1. Если данные содержат ответ — отвечай одним предложением с конкретной цифрой. Пример: «Выручка Монблан в апреле: 4 823 500 ₽». "
                "2. Если данных за нужный период нет или вопрос неоднозначен — задай уточняющий вопрос пользователю. Например: «За какую неделю апреля?» или «Имеется в виду общая выручка или по залам?». "
                "3. Никаких рассуждений, процентов и лишних деталей — только ответ или один уточняющий вопрос."
            )},
...
def query_sheets_sync(question: str) -> str:
    ...
    if any(k in q for k in ["монблан", "ресторан", "monblan"]):
        if any(w in q for w in ["месяц", "май", "июн", "июл", "август", "январ", "феврал"]):
